# ASPAProj — инструмент расчёта двухпоточного теплообмена
...
- Расчёт тепловой нагрузки `Q`, выхода горячего потока `T⁺out`, коэффициента теплопередачи `K` (UA эквивалент) и удельной скорости производства энтропии `σ`.
...
```powershell
schtasks /Create /TN AspaProjBackup /TR "pwsh -NoProfile -ExecutionPolicy Bypass -File C:\path\to\backup.ps1" /SC MINUTE /MO 30 /F
```
